  ...
  useEffect(() => {
    if (toolId) {
      ...
      if (tool) {
        ...
        // Record tool access as an action
        recordAction({
          toolId: tool.id,
          type: 'input',
          data: { action: 'tool_opened', toolName: tool.name },
          nextPossibleActions: []
        });
      }
    }
  }, [toolId, addRecentTool, recordAction]);

  // Handle data updates from tools
  const handleToolDataUpdate = (data: any) => {
    setToolData(data);

    // Update workspace data
    updateWorkspaceData(toolId, data);

    // Set context based on data type
    if (typeof data === 'string' && data.length > 10) {
      setContext('text', data);
    } else if (data && typeof data === 'object' && data.url) {
      setContext('image', data);
